...
导入资源文件夹用于把外部资源目录转换成一个新的 EffectViewer 项目。它不是把文件夹追加到当前项目，而是创建并加载一个新内部项目。

导入前行为：

- 如果当前项目有未保存更改，会先询问是否保存、放弃或取消。
- 导入成功后，当前工作区会切换到新导入的项目。
...
- 编译版 reanim、particle 和 trail 会转换为源格式保存到项目中。
...
用途：把 `.pak` 资源包当作资源文件夹读取并转换成新的 EffectViewer 项目。
...
拖放导入单个资源时需要当前已有可写项目；拖放 `.pak` 会按 Pak 导入流程创建新项目。
...
这是预期行为。资源文件夹导入是“转换外部资源目录为新项目”，不是追加文件夹到当前项目。若要追加资源，请使用 `资源 -> 导入资源文件...`。

...
namespace EffectViewer.EffectRuntime.Reanim
{
    public class Reanimation : IDataArrayItem
    {
        ...
        public EffectColor mExtraAdditiveColor;
        public bool mEnableExtraAdditiveDraw;
        public EffectColor mExtraOverlayColor;
        ...
        public void Reset()
        {
            ...
            mExtraAdditiveColor = EffectColor.White;
            mEnableExtraAdditiveDraw = false;
            mExtraOverlayColor = EffectColor.White;
        ...
        public void ReanimationInitialize(float theX, float theY, string theDefinition)
        {
            ...
            mDefinition = ReanimatorUtility.gReanimatorDefArray[theDefinition];
            ...
            if (mDefinition.mTrackCount != 0)
            {
                ...
                for (int aTrackIndex = 0; aTrackIndex < mDefinition.mTrackCount; aTrackIndex++)  // 遍历初始化数组中每个轨道实例
                {
                    ...
                    mTrackInstances[aTrackIndex].mIsAttacher =
                        ...
                        aName.StartsWith(Attacher, StringComparison.OrdinalIgnoreCase);
                }
            }
            else
            {
        ...
        {
            ReanimatorUtility.ReanimatorEnsureDefinitionLoaded(theReanimType, false);
        ...
        public void Update()
        {
            ...
            mAnimTime += ReanimatorUtility.SECONDS_PER_UPDATE * mAnimRate / mFrameCount;  // 更新当前循环率
            ...
            for (int aTrackIndex = 0; aTrackIndex < mDefinition.mTrackCount; aTrackIndex++)
            {
                ...
                {
                    aTrack.mShakeX = EffectUtility.RandRangeFloat(-aTrack.mShakeOverride, aTrack.mShakeOverride);
                    aTrack.mShakeY = EffectUtility.RandRangeFloat(-aTrack.mShakeOverride, aTrack.mShakeOverride);
                }
        ...
        {
            DrawRenderGroup(g, ReanimatorUtility.RENDER_GROUP_NORMAL);
        }

        public void DrawRenderGroup(EffectViewer.EffectRuntime.Graphics.Graphics g, int theRenderGroup)
        {
            if (mDead)
            {
                return;
        ...
        public bool DrawTrack(EffectViewer.EffectRuntime.Graphics.Graphics g, int theTrackIndex, int theRenderGroup)
        {
            ...
            int aImageFrame = EffectUtility.FloatRoundToInt(aTransform.mFrame);  // 图像在贴图中所处的份数
            if (aImageFrame < 0f)  // 不存在图像时，返回
            {
                return false;
            }

            EffectColor aColor = aTrackInstance.mTrackColor;
            if (!aTrackInstance.mIgnoreColorOverride)  // 除非轨道无视动画的覆写颜色
            {
                aColor = EffectUtility.ColorsMultiply(aColor, mColorOverride);  // 将轨道颜色与动画的覆写颜色进行正片叠底混合
            }
            if (g.GetColorizeImages())  // 若 Graphics 着色
            {
                aColor = EffectUtility.ColorsMultiply(aColor, g.mColor);  // 将颜色再与 Graphics 的颜色进行正片叠底混合
            }

            int aImageAlpha = EffectUtility.ClampInt(EffectUtility.FloatRoundToInt(aTransform.mAlpha * aColor.mAlpha), 0, 255);
            ...
            EffectColor aExtraAdditiveColor = EffectColor.Black;
            if (mEnableExtraAdditiveDraw)  // 如果动画启用额外叠加颜色（高亮）
            {
                aExtraAdditiveColor = mExtraAdditiveColor;
                aExtraAdditiveColor.mAlpha = EffectUtility.ColorComponentMultiply(mExtraAdditiveColor.mAlpha, aImageAlpha);
            }

            EffectColor aExtraOverlayColor = EffectColor.Black;
            if (mEnableExtraOverlayDraw)
            {
                aExtraOverlayColor = mExtraOverlayColor;
                aExtraOverlayColor.mAlpha = EffectUtility.ColorComponentMultiply(mExtraOverlayColor.mAlpha, aImageAlpha);
            }
            ...
            if (aImage != null)  // 如果存在贴图。此处若上一步中图集编号非法，则可能导致崩溃
            {
                ...
                EffectUtility.Matrix3Translation(ref aMatrix, aCelWidth * 0.5f, aCelHeight * 0.5f);  // 将矩阵变换的坐标设定在贴图的中心位置
            }
            else if (aTransform.mFont != null && !string.IsNullOrEmpty(aTransform.mText))  // 如果存在字体且文本不为空
            {
                ...
                EffectUtility.Matrix3Translation(ref aMatrix, -aWidth * 0.5f, aFont.mAscent);
            }
            else
            {
            ...
            EffectUtility.Matrix3Multiply(ref aMatrix, aTransformMatrix, aMatrix);  // 以动画变换矩阵作用 aMatrix
            EffectUtility.Matrix3Multiply(ref aMatrix, mOverlayMatrix, aMatrix);  // 以动画覆写矩阵作用 aMatrix
            EffectUtility.Matrix3Translation(ref aMatrix, aTrackInstance.mShakeX + g.mTransX - 0.5f, aTrackInstance.mShakeY + g.mTransY - 0.5f);  // 轨道震动及 g 的影响
            ...
            else if ((!string.IsNullOrEmpty(aTrackInstance.mFontOverride) || aTransform.mFont != null) && !string.IsNullOrEmpty(aTransform.mText))  // 如果不存在图像但存在文本
            {
                ...
                EffectUtility.DrawStringMatrix(g, aFont, aMatrix, aTransform.mText, aColor);
                if (mEnableExtraAdditiveDraw)
                {
                    DrawMode aOldMode = g.GetDrawMode();  // 备份绘制模式
                    g.SetDrawMode(DrawMode.Additive);
                    EffectUtility.DrawStringMatrix(g, aFont, aMatrix, aTransform.mText, aExtraAdditiveColor);
                    g.SetDrawMode(aOldMode);  // 还原绘制模式
                }
            }
            else if (aFullScreen)  // 不存在图像和文本，但是全屏
            {
                EffectColor aOldColor = g.GetColor();  // 备份颜色
                g.SetColor(aColor);
        ...
        public void GetCurrentTransform(int theTrackIndex, out ReanimatorTransform theTransformCurrent)
        {
            ...
            if (EffectUtility.FloatRoundToInt(theTransformCurrent.mFrame) >= 0 && aTrack.mBlendCounter > 0)  // 若当前不为空白帧且轨道处于变换混合过程中
            {
                float aBlendFactor = aTrack.mBlendCounter / (float)aTrack.mBlendTime;
                ReanimatorUtility.BlendTransform(ref theTransformCurrent, theTransformCurrent, aTrack.mBlendTransform, aBlendFactor);
            }
        }
        ...
        public void GetTransformAtTime(int theTrackIndex, out ReanimatorTransform theTransform, in ReanimatorFrameTime theFrameTime)
        {
            ...
            theTransform.mTransX = EffectUtility.FloatLerp(aTransBefore.mTransX, aTransAfter.mTransX, theFrameTime.mFraction);
            theTransform.mTransY = EffectUtility.FloatLerp(aTransBefore.mTransY, aTransAfter.mTransY, theFrameTime.mFraction);
            theTransform.mSkewX = EffectUtility.FloatLerp(aTransBefore.mSkewX, aTransAfter.mSkewX, theFrameTime.mFraction);
            theTransform.mSkewY = EffectUtility.FloatLerp(aTransBefore.mSkewY, aTransAfter.mSkewY, theFrameTime.mFraction);
            theTransform.mScaleX = EffectUtility.FloatLerp(aTransBefore.mScaleX, aTransAfter.mScaleX, theFrameTime.mFraction);
            theTransform.mScaleY = EffectUtility.FloatLerp(aTransBefore.mScaleY, aTransAfter.mScaleY, theFrameTime.mFraction);
            theTransform.mAlpha = EffectUtility.FloatLerp(aTransBefore.mAlpha, aTransAfter.mAlpha, theFrameTime.mFraction);
        ...
        public void GetFrameTime(out ReanimatorFrameTime theFrameTime)
        {
            ...
            theFrameTime.mAnimFrameBeforeInt = (short)EffectUtility.FloatRoundToInt(aAnimFrameBefore);
        ...
        public void GetAttachmentOverlayMatrix(int theTrackIndex, out Matrix4x4 theOverlayMatrix)
        {
            ...
            MatrixFromTransform(aTransform, out Matrix4x4 aTransformMatrix);
            EffectUtility.Matrix3Multiply(ref aTransformMatrix, mOverlayMatrix, aTransformMatrix);  // 以动画覆写矩阵作用于动画变换矩阵

            GetTrackBasePoseMatrix(theTrackIndex, out Matrix4x4 aBasePoseMatrix);  // 取得轨道基础形态的变换矩阵
            EffectUtility.Matrix3Inverse(aBasePoseMatrix, out Matrix4x4 aBasePoseMatrixInv);  // 取得基础形态矩阵的逆
        ...
        {
            // 将倾斜的角度转化为弧度
            float aSkewX = -EffectUtility.DEG_TO_RAD * theTransform.mSkewX;
            float aSkewY = -EffectUtility.DEG_TO_RAD * theTransform.mSkewY;
        ...
        public void StartBlend(byte theBlendTime)
        {
            for (int aTrackIndex = 0; aTrackIndex < mDefinition.mTrackCount; aTrackIndex++)
            {
                ...
                if (EffectUtility.FloatRoundToInt(aTransform.mFrame) >= 0)  // 若当前轨道当前不处于空白帧
                {
                    ref ReanimatorTrackInstance aTrackInstance = ref mTrackInstances[aTrackIndex];
        ...
        public float GetTrackVelocity(string theTrackName)
        {
            ...
            return aDis * ReanimatorUtility.SECONDS_PER_UPDATE * mAnimRate;  // 瞬时速率 = 两帧间的横坐标之差 * 一帧的时长 * 动画速率
        }
        ...
        public void GetTrackMatrix(int theTrackIndex, out Matrix4x4 theMatrix)
        {
            ...
            int aImageFrame = EffectUtility.FloatRoundToInt(aTransform.mFrame);
            ...
            if (aImage != null && aImageFrame >= 0)
            {
                int aCelWidth = aImage.GetCelWidth();
                int aCelHeight = aImage.GetCelHeight();
                EffectUtility.Matrix3Translation(ref theMatrix, aCelWidth * 0.5f, aCelHeight * 0.5f);  // 将矩阵变换的坐标设定在贴图的中心位置
            }
            else if (aTransform.mFont != null && !string.IsNullOrEmpty(aTransform.mText))
            {
                ...
                if (aFont != null)
                {
                    EffectUtility.Matrix3Translation(ref theMatrix, 0f, aFont.mAscent);
                }
            }

            MatrixFromTransform(aTransform, out Matrix4x4 aTransformMatrix);
            EffectUtility.Matrix3Multiply(ref theMatrix, aTransformMatrix, theMatrix);  // 以动画变换矩阵作用 theMatrix
            EffectUtility.Matrix3Multiply(ref theMatrix, mOverlayMatrix, theMatrix);  // 以动画覆写矩阵作用 theMatrix
            EffectUtility.Matrix3Translation(ref theMatrix, aTrackInstance.mShakeX - 0.5f, aTrackInstance.mShakeY - 0.5f);  // 轨道震动的影响
        }
        ...
        public ref AttachEffect AttachParticleToTrack(string theTrackName, ParticleSystem theParticleSystem, float thePosX, float thePosY)
        {
        ...
        public void GetTrackBasePoseMatrix(int theTrackIndex, out Matrix4x4 theBasePoseMatrix)
        {
            ...
            if (mFrameBasePose == ReanimatorUtility.NO_BASE_POSE)
            {
                return;
            }
        ...
        public void UpdateAttacherTrack(int theTrackIndex)
        {
            ...
            if (aAttachReanim == null || aAttachReanim.mReanimationType != aReanimationType)  // 如果原先没有附属动画，或原附属动画不是上述设定的动画
            {
                ...
                mFrameBasePose = ReanimatorUtility.NO_BASE_POSE;  // 设定附属动画后，自身不再存在基准帧
            }
            ...
            EffectColor aColor = EffectUtility.ColorsMultiply(mColorOverride, aTrackInstance.mTrackColor);
            aColor.mAlpha = EffectUtility.ClampInt(EffectUtility.FloatRoundToInt(aTransform.mAlpha * aColor.mAlpha), 0, 255);
        ...
        private static string GetAttacherReanimationType(AttacherInfo theAttacherInfo)
        {
            if (theAttacherInfo.mReanimName.Length == 0 ||
                ReanimatorUtility.gReanimationParamArray is null)
            {
                return null;
            }

            string aReanimFileName = $"reanim/{theAttacherInfo.mReanimName}";
            foreach ((_, ReanimationParams aParams) in ReanimatorUtility.gReanimationParamArray)
            {
        ...
        public void AttacherSynchWalkSpeed(int theTrackIndex, ref Reanimation theAttachReanim, ref AttacherInfo theAttacherInfo)
        {
            ...
            if (EffectUtility.FloatApproxEqual(mAnimRate, 0f))  // 如果动画自身的速率为 0
            {
            ...
            if (EffectUtility.FloatApproxEqual(aPlaceHolderSeconds, 0f))  // 如果当前所在区间不存在任何帧
            {
            ...
            if (aGuyDistance < ReanimatorUtility.EPSILON || aPlaceHolderDistance < ReanimatorUtility.EPSILON)  // 如果占位位移为 0 或实际动画周期位移为 0，则附属动画无法移动
            {
                theAttachReanim.mAnimRate = 0f;  // 附属动画的速率为 0
                return;
            }
        ...
        public void ReanimBltMatrix(EffectViewer.EffectRuntime.Graphics.Graphics g, Image theImage, in Matrix4x4 theTransform, in Rectangle theClipRect, in EffectColor theColor, DrawMode theDrawMode, in Rectangle theSrcRect)
        {
            EffectUtility.BlitMatrix(g, theImage, theTransform, theClipRect, theColor, theDrawMode, theSrcRect);
        }

...
namespace EffectViewer.EffectRuntime.Trail
{
    ...
    {
        public InlineArray20<TrailPoint> mTrailPoints; // EffectConstants.MAX_TRAIL_POINTS
        ...
        public void Reset()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                mTrailInterp[i] = EffectUtility.RandRangeFloat(0f, 1f);
            }
        ...
        public void Update()
        {
            ...
            {
                if (EffectUtility.TestBit((uint)mDefinition.mTrailFlags, (int)TrailFlags.Loops))
                {
                    mTrailAge = 0;
                }
                else
        ...
        public void Draw(EffectViewer.EffectRuntime.Graphics.Graphics g)
        {
            ...
            Debug.Assert(aTriangleCount <= EffectConstants.MAX_TRAIL_TRIANGLES);

            InlineArray38<InlineArray3<TriVertex>> aVertArray = new(); // EffectConstants.MAX_TRAIL_TRIANGLES
            ...
            for (int i = 0; i < mNumTrailPoints - 1; i++)
            {
                ...
                int anAlphaCur = EffectUtility.ClampInt(EffectUtility.FloatRoundToInt(aAlphaOverLengthCur * aAlphaOverTimeCur * mColorOverride.mAlpha), 0, 255);
                int anAlphaNext = EffectUtility.ClampInt(EffectUtility.FloatRoundToInt(aAlphaOverLengthNext * aAlphaOverTimeNext * mColorOverride.mAlpha), 0, 255);
                EffectColor aColorCur = mColorOverride;
                EffectColor aColorNext = mColorOverride;
        ...
        {
            int aMaxPoints = EffectUtility.ClampInt(mDefinition.mMaxPoints, 2, EffectConstants.MAX_TRAIL_POINTS);
            if (mNumTrailPoints > 0)
            {
                ref TrailPoint aPoint = ref mTrailPoints[mNumTrailPoints - 1];
                float aDistance = EffectUtility.Distance2D(x, y, aPoint.aPos.X, aPoint.aPos.Y);
        ...
        public bool GetNormalAtPoint(int nIndex, ref Vector2 theNormal)
        {
            ...
            if (EffectUtility.FloatApproxEqual(aMag, 0f))
            {
                return false;
            }

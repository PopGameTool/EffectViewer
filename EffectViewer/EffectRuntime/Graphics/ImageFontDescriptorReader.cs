...
namespace EffectViewer.EffectRuntime.Graphics
{
    public static class ImageFontDescriptorReader
    {
        ...
        private static EffectColor ParseColor(string value)
        {
            string trimmed = value?.Trim() ?? string.Empty;
            if (trimmed.StartsWith("(", StringComparison.Ordinal))
            {
                ...
                if (values.Count >= 4)
                {
                    return new EffectColor(values[0], values[1], values[2], values[3]);
                }
            }
            ...
            return new EffectColor((color >> 16) & 0xFF, (color >> 8) & 0xFF, color & 0xFF, alpha);
        }

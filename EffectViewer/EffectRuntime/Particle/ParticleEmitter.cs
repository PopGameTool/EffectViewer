...
{
    public class ParticleEmitter : IDataArrayItem
    {
        ...
        public bool mDead;
        public EffectColor mColorOverride;
        ...
        public InlineArray4<InlineArray2<float>> mSystemFieldInterp; // EffectConstants.MAX_PARTICLE_FIELDS
        ...
        public void InitializeEmitter(float theX, float theY, ParticleSystem theSystem, ParticleEmitterDefinition theEmitterDef)
        {
            mSpawnAccum = 0f;
            mParticlesSpawned = 0;
            mSystemTimeValue = -1f;
            ...
            mSystemAge = -1;
            mDead = false;
            mColorOverride = EffectColor.White;
        ...
        public void Update()
        {
            ...
            {
                if (EffectUtility.TestBit((uint)mEmitterDef.mParticleFlags, (int)ParticleFlags.SystemLoops))  // 判断发射器是否循环
                {
            ...
            {
                ParticleEmitter aCrossFadeEmitter = mParticleSystem.mParticleHolder.mEmitters.DataArrayTryToGet(mCrossFadeEmitterID);
            ...
            {
                ParticleInstance aParticle = mParticleSystem.mParticleHolder.mParticles.DataArrayGet(aNode.Value);
                aNode = aNode.Next;
        ...
        public void Draw(EffectViewer.EffectRuntime.Graphics.Graphics g)
        {
            ...
            if (EffectUtility.TestBit((uint)mEmitterDef.mParticleFlags, (int)ParticleFlags.SoftwareOnly) &&
                aHardWare)
            {
                return;
            }

            if (EffectUtility.TestBit((uint)mEmitterDef.mParticleFlags, (int)ParticleFlags.HardwareOnly) &&
                !aHardWare)
            {
                return;
            }
        ...
        public void SystemMove(float theX, float theY)
        {
            ...
            if (EffectUtility.FloatApproxEqual(aDeltaX, 0f) &&
                EffectUtility.FloatApproxEqual(aDeltaY, 0f))  // 移动前后的坐标几乎相等时，无需操作
            {
                return;
            }
            ...
            mSystemCenter.X = theX;
            mSystemCenter.Y = theY;
            if (!EffectUtility.TestBit((uint)mEmitterDef.mParticleFlags, (int)ParticleFlags.ParticlesDontFollow))
            {
                for (LinkedListNode<ParticleID> aNode = mParticleList.First; aNode != null; aNode = aNode.Next)
                {
                    ParticleInstance aParticle = mParticleSystem.mParticleHolder.mParticles.DataArrayGet(aNode.Value);
        ...
        public static bool GetRenderParams(ParticleInstance theParticle, ref ParticleRenderParams theParams)
        {
            ParticleEmitter aEmitter = theParticle.mParticleEmitter;
            ...
            theParams.mSpinPositionIsSet |= EffectUtility.TestBit((uint)aDef.mParticleFlags, (int)ParticleFlags.RandomLaunchSpin);
            theParams.mSpinPositionIsSet |= EffectUtility.TestBit((uint)aDef.mParticleFlags, (int)ParticleFlags.AlignLaunchSpin);
            // 位置
            ...
            ParticleInstance aCrossFadeParticle = aEmitter.mParticleSystem.mParticleHolder.mParticles.DataArrayTryToGet(theParticle.mCrossFadeParticleID);
            if (aCrossFadeParticle != null)  // 当存在交叉混合的粒子时，将二者的渲染参数进行混合（从 aCrossFadeParticle 至 theParticle 的交叉混合）
            {
                ...
                if (GetRenderParams(aCrossFadeParticle, ref aCrossFadeParams))
                {
                    ...
                    theParams.mRed = ParticleUtility.CrossFadeLerp(aCrossFadeParams.mRed, theParams.mRed, aCrossFadeParams.mRedIsSet, theParams.mRedIsSet, aFraction);
                    theParams.mGreen = ParticleUtility.CrossFadeLerp(aCrossFadeParams.mGreen, theParams.mGreen, aCrossFadeParams.mGreenIsSet, theParams.mGreenIsSet, aFraction);
                    theParams.mBlue = ParticleUtility.CrossFadeLerp(aCrossFadeParams.mBlue, theParams.mBlue, aCrossFadeParams.mBlueIsSet, theParams.mBlueIsSet, aFraction);
                    theParams.mAlpha = ParticleUtility.CrossFadeLerp(aCrossFadeParams.mAlpha, theParams.mAlpha, aCrossFadeParams.mAlphaIsSet, theParams.mAlphaIsSet, aFraction);
                    theParams.mParticleScale = ParticleUtility.CrossFadeLerp(aCrossFadeParams.mParticleScale, theParams.mParticleScale, aCrossFadeParams.mParticleScaleIsSet, theParams.mParticleScaleIsSet, aFraction);
                    theParams.mParticleStretch = ParticleUtility.CrossFadeLerp(aCrossFadeParams.mParticleStretch, theParams.mParticleStretch, aCrossFadeParams.mParticleStretchIsSet, theParams.mParticleStretchIsSet, aFraction);
                    theParams.mSpinPosition = ParticleUtility.CrossFadeLerp(aCrossFadeParams.mSpinPosition, theParams.mSpinPosition, aCrossFadeParams.mSpinPositionIsSet, theParams.mSpinPositionIsSet, aFraction);
                    theParams.mPosX = ParticleUtility.CrossFadeLerp(aCrossFadeParams.mPosX, theParams.mPosX, aCrossFadeParams.mPositionIsSet, theParams.mPositionIsSet, aFraction);
                    theParams.mPosY = ParticleUtility.CrossFadeLerp(aCrossFadeParams.mPosY, theParams.mPosY, aCrossFadeParams.mPositionIsSet, theParams.mPositionIsSet, aFraction);
        ...
        public void DrawParticle(EffectViewer.EffectRuntime.Graphics.Graphics g, ParticleInstance theParticle)
        {
            ...
            {
                EffectColor aColor = new(
                    EffectUtility.ClampInt(EffectUtility.FloatRoundToInt(aParams.mRed), 0, 255),
                    EffectUtility.ClampInt(EffectUtility.FloatRoundToInt(aParams.mGreen), 0, 255),
                    EffectUtility.ClampInt(EffectUtility.FloatRoundToInt(aParams.mBlue), 0, 255),
                    EffectUtility.ClampInt(EffectUtility.FloatRoundToInt(aParams.mAlpha), 0, 255)
                    );
                if (aColor.mAlpha > 0)  // 不透明度为 0 时，不绘制
                {
                    ...
                    if (aParticle != null)
                    {
                        ParticleUtility.RenderParticle(g, aParticle, aColor, ref aParams);
                    }
                }
            }
        }
        ...
        {
            ParticleEmitter aCrossFadeEmitter = mParticleSystem.mParticleHolder.mEmitters.DataArrayTryToGet(mCrossFadeEmitterID);
            ParticleEmitter aSpawningEmitter = aCrossFadeEmitter ?? this;  // 各项数据的计算均以此“主发射器”为准
            ...
            {
                ParticleInstance aParticle = SpawnParticle(i, aSpawnCount);
        ...
        public bool UpdateParticle(ParticleInstance theParticle)
        {
            if (theParticle.mParticleAge >= theParticle.mParticleDuration)  // 粒子的生命周期结束时
            {
                if (EffectUtility.TestBit((uint)mEmitterDef.mParticleFlags, (int)ParticleFlags.ParticleLoops))  // 判断粒子是否循环
                {
            ...
            float aLastSpinAngle = ParticleUtility.FloatTrackEvaluateFromLastTime(mEmitterDef.mParticleSpinAngle, theParticle.mParticleLastTimeValue, theParticle.mParticleInterp[(int)ParticleTracks.ParticleSpinAngle]);
            theParticle.mSpinPosition += EffectUtility.DegToRad(aSpinSpeed + aSpinAngle - aLastSpinAngle) + theParticle.mSpinVelocity;  // 更新粒子旋转角度
        ...
        public ParticleInstance SpawnParticle(int theIndex, int theSpawnCount)
        {
            DataArray<ParticleInstance, ParticleID> aDataArray = mParticleSystem.mParticleHolder.mParticles;
            ...
            ParticleInstance aParticle = aDataArray.DataArrayAlloc();
            Debug.Assert(mEmitterDef.mParticleFieldCount <= EffectConstants.MAX_PARTICLE_FIELDS);
            ...
            if (EffectUtility.TestBit((uint)mEmitterDef.mParticleFlags, (int)ParticleFlags.RandomStartTime))
            {
            ...
            if (mEmitterDef.mEmitterType == EmitterType.CirclePath)
            {
                ...
                aLaunchAngle += EffectUtility.DegToRad(Definition.FloatTrackEvaluate(mEmitterDef.mLaunchAngle, mSystemTimeValue, aLaunchAngleInterp));
            }
            else if (mEmitterDef.mEmitterType == EmitterType.CircleEvenSpacing)
            {
                // 基础发射角度要使 theSpawnCount 个粒子平均布满圆周
                aLaunchAngle = MathF.Tau * theIndex / theSpawnCount + EffectUtility.DegToRad(Definition.FloatTrackEvaluate(mEmitterDef.mLaunchAngle, mSystemTimeValue, aLaunchAngleInterp));
            }
            ...
            if (EffectUtility.TestBit((uint)mEmitterDef.mParticleFlags, (int)ParticleFlags.RandomLaunchSpin))
            {
                aParticle.mSpinPosition = RandomNumbers.Rand(MathF.Tau);  // 在 [0, 2pi] 之间随机取得一个初始旋转角度
            }
            else if (EffectUtility.TestBit((uint)mEmitterDef.mParticleFlags, (int)ParticleFlags.AlignLaunchSpin))
            {
        ...
        public bool CrossFadeParticle(ParticleInstance theParticle, ParticleEmitter theToEmitter)
        {
            ...
            ParticleInstance aToParticle = theToEmitter.SpawnParticle(0, 1);
            if (aToParticle == null)
            {
                return false;
            }
        ...
        public void CrossFadeEmitter(ParticleEmitter theToEmitter)
        {
            if (mEmitterCrossFadeCountDown > 0)
            {
        ...
        public bool CrossFadeParticleToName(ParticleInstance theParticle, string theEmitterName)
        {
            ParticleEmitterDefinition aDef = mParticleSystem.FindEmitterDefByName(theEmitterName);
            if (aDef == null)
            {
            ...
            ParticleEmitter aEmitter = mParticleSystem.mParticleHolder.mEmitters.DataArrayAlloc();
            aEmitter.InitializeEmitter(mSystemCenter.X, mSystemCenter.Y, mParticleSystem, aDef);
        ...
        public void UpdateParticleField(ParticleInstance theParticle, ParticleField theParticleField,
            float theParticleTimeValue, int theFieldIndex)
        {
            Debug.Assert(theFieldIndex < EffectConstants.MAX_PARTICLE_FIELDS);
            ...
            case ParticleFieldType.MaxVelocity:  // 限速场
                theParticle.mVelocity.X = EffectUtility.ClampFloat(theParticle.mVelocity.X, -x, x);
                theParticle.mVelocity.Y = EffectUtility.ClampFloat(theParticle.mVelocity.Y, -y, y);
                break;
            ...
            {
                float aLastX = ParticleUtility.FloatTrackEvaluateFromLastTime(theParticleField.mX, theParticle.mParticleLastTimeValue, aInterpX);
                float aLastY = ParticleUtility.FloatTrackEvaluateFromLastTime(theParticleField.mY, theParticle.mParticleLastTimeValue, aInterpY);
        ...
        public void UpdateSystemField(ParticleField theParticleField, float theParticleTimeValue, int theFieldIndex)
        {
            ...
            {
            case ParticleFieldType.SystemPosition:
                float aLastX = ParticleUtility.FloatTrackEvaluateFromLastTime(theParticleField.mX, mSystemLastTimeValue, aInterpX);
                float aLastY = ParticleUtility.FloatTrackEvaluateFromLastTime(theParticleField.mY, mSystemLastTimeValue, aInterpY);
        ...
        public static float ParticleTrackEvaluate(FloatParameterTrack theTrack, ParticleInstance theParticle, ParticleTracks theParticleTrack)
        {
        ...
        public void DeleteParticle(ParticleInstance theParticle)
        {
            ParticleInstance aCrossFadeParticle = mParticleSystem.mParticleHolder.mParticles.DataArrayTryToGet(theParticle.mCrossFadeParticleID);

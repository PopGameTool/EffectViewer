...
namespace EffectViewer.EffectRuntime.Particle
{
    public static class ParticleUtility
    {
        ...
        public static Dictionary<string, ParticleDefinition> gParticleDefArray;
        public static int gParticleParamArraySize;
        public static ParticleParams[] gParticleParamArray;

        public static void LoadDefinitions(ref ParticleParams[] theParticleParamArray,
            int theParticleParamArraySize)
        {
            ...
            gParticleDefArray = new Dictionary<string, ParticleDefinition>();

            for (int i = 0; i < gParticleParamArraySize; i++)
            {
                ParticleParams aParticleParams = gParticleParamArray[i];
                if (!LoadDefinition(out ParticleDefinition aDef, aParticleParams.mParticleFileName))
                {
        ...
        public static void FreeDefinitions()
        {
        ...
        public static bool LoadDefinition(out ParticleDefinition theParticleDef, string aFilename)
        {
            using (Stream fileStream = File.OpenRead(aFilename))
            {
                theParticleDef = ParticleDefinitionCodec.Decode(fileStream);
            }

            for (int i = 0; i < theParticleDef.mEmitterDefCount; i++)
            {
                ParticleEmitterDefinition aDef = theParticleDef.mEmitterDefs[i];
        ...
        public static void RenderParticle(EffectViewer.EffectRuntime.Graphics.Graphics g, ParticleInstance theParticle, EffectColor theColor, ref ParticleRenderParams theParams)
        {
            ParticleEmitter aEmitter = theParticle.mParticleEmitter;
            ParticleEmitterDefinition aEmitterDef = aEmitter.mEmitterDef;
            ...
            {
                aFrame = EffectUtility.ClampInt((int)(theParticle.mAnimationTimeValue * aEmitterDef.mImageFrames), 0, aEmitterDef.mImageFrames - 1);  // 动画时间值（循环率） * 总帧数得到当前帧
            }
            else if (aEmitterDef.mAnimated != 0)
            {
                aFrame = EffectUtility.ClampInt((int)(theParticle.mParticleTimeValue * aEmitterDef.mImageFrames), 0, aEmitterDef.mImageFrames - 1);  // 粒子时间值 * 总帧数得到当前帧
            }
            else
            {
            ...
            float aClipTop = ParticleEmitter.ParticleTrackEvaluate(aEmitterDef.mClipTop, theParticle, ParticleTracks.ParticleClipTop);
            float aClipBottom = ParticleEmitter.ParticleTrackEvaluate(aEmitterDef.mClipBottom, theParticle, ParticleTracks.ParticleClipBottom);
            float aClipLeft = ParticleEmitter.ParticleTrackEvaluate(aEmitterDef.mClipLeft, theParticle, ParticleTracks.ParticleClipLeft);
            float aClipRight = ParticleEmitter.ParticleTrackEvaluate(aEmitterDef.mClipRight, theParticle, ParticleTracks.ParticleClipRight);
            ...
            aSrcRect.X += EffectUtility.FloatRoundToInt(aClipLeft * aCelWidth);
            aSrcRect.Y += EffectUtility.FloatRoundToInt(aClipTop * aCelHeight);
            aSrcRect.Width -= EffectUtility.FloatRoundToInt((aClipLeft + aClipRight) * aCelWidth);
            aSrcRect.Height -= EffectUtility.FloatRoundToInt((aClipTop + aClipBottom) * aCelHeight);  // 以上根据裁剪各方向的比例调整源矩形
            Debug.Assert(aSrcRect.X == aCelWidth * aFrame + EffectUtility.FloatRoundToInt(aClipLeft * aCelWidth));
            Debug.Assert(aSrcRect.Y == aCelHeight * aEmitterDef.mImageRow + EffectUtility.FloatRoundToInt(aClipTop * aCelHeight));
            Debug.Assert(aSrcRect.X >= 0 && aSrcRect.X < 10000);
            Debug.Assert(aSrcRect.Y >= 0 && aSrcRect.Y < 10000);

            if (EffectUtility.TestBit((uint)aEmitterDef.mParticleFlags, (int)ParticleFlags.AlignToPixels))  // 坐标对齐至整数像素点
            {
                theParams.mPosX = EffectUtility.FloatRoundToInt(theParams.mPosX);
                theParams.mPosY = EffectUtility.FloatRoundToInt(theParams.mPosY);
            }

            DrawMode aDrawMode = g.mDrawMode;
            if (EffectUtility.TestBit((uint)aEmitterDef.mParticleFlags, (int)ParticleFlags.Additive))  // 使用叠加模式
            {
                aDrawMode = DrawMode.Additive;
            }

            if (EffectUtility.TestBit((uint)aEmitterDef.mParticleFlags, (int)ParticleFlags.Fullscreen))  // 全屏模式
            {
                EffectColor anOldColor = g.GetColor();
            ...
            else
            {
                ...
                EffectUtility.ScaleRotateTransformMatrix(
                    ref aTransform,
                    theParams.mPosX,
                    theParams.mPosY,
                ...
                EffectUtility.BlitMatrix(g, aImage, aTransform, g.GetClipRect(), theColor, aDrawMode, aSrcRect);
                if (aEmitter.mExtraAdditiveDrawOverride)
                {
                    EffectUtility.BlitMatrix(g, aImage, aTransform, g.GetClipRect(), theColor, DrawMode.Additive, aSrcRect);
                }
            }
        }
    }

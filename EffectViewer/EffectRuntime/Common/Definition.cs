namespace EffectViewer.EffectRuntime.Common
{
    internal static class Definition
    {
        public static float FloatTrackEvaluate(FloatParameterTrack theTrack, float theTimeValue, float theInterp)
        {
            ...
            {
                return EffectUtility.EvaluateCurve(theInterp, theTrack.mNodes[0].mLowValue, theTrack.mNodes[0].mHighValue, theTrack.mNodes[0].mDistribution);
            }
            ...
            for (int i = 1; i < theTrack.mCountNodes; i++)
            {
                ...
                if (theTimeValue <= aNodeNxt.mTime)  // 寻找首个开始时间大于当前时间的节点
                {
                    ...
                    float aLeftValue = EffectUtility.EvaluateCurve(theInterp, aNodeCur.mLowValue, aNodeCur.mHighValue, aNodeCur.mDistribution);
                    float aRightValue = EffectUtility.EvaluateCurve(theInterp, aNodeNxt.mLowValue, aNodeNxt.mHighValue, aNodeNxt.mDistribution);
                    return EffectUtility.EvaluateCurve(aTimeFraction, aLeftValue, aRightValue, aNodeCur.mCurveType);
                }
            }

            FloatParameterTrackNode aLastNode = theTrack.mNodes[theTrack.mCountNodes - 1];  // 如果当前时间大于最后一个节点的开始时间
            return EffectUtility.EvaluateCurve(theInterp, aLastNode.mLowValue, aLastNode.mHighValue, aLastNode.mDistribution);
        }
        ...
        public static bool FloatTrackIsSet(FloatParameterTrack theTrack)
        {
            return theTrack.mCountNodes != 0 &&
                theTrack.mNodes[0].mCurveType != CurveType.Constant;
        }

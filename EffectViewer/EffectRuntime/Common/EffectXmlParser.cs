...
{
    internal enum EffectXmlElementType
    {
        None,
        Start,
        End,
    ...
    internal sealed class EffectXmlElement
    {
        public EffectXmlElementType Type { get; init; }
    ...
    internal sealed class EffectXmlParser
    {
        private readonly string _text;
        private readonly Queue<EffectXmlElement> _pending = new();
        private readonly List<string> _sections = [];
        private int _position;

        private EffectXmlParser(string text, string fileName)
        {
        ...
        public static EffectXmlParser FromStream(Stream stream, string fileName = "")
        {
            using StreamReader reader = new(stream, Encoding.UTF8, detectEncodingFromByteOrderMarks: true, leaveOpen: true);
            return new EffectXmlParser(reader.ReadToEnd(), fileName);
        }

        public static EffectXmlParser FromFile(string fileName)
        {
            using FileStream stream = File.OpenRead(fileName);
            return FromStream(stream, fileName);
        }

        public bool TryNextElement(out EffectXmlElement element)
        {
            if (_pending.Count > 0)
            {
        ...
        private bool TryReadText(out EffectXmlElement element)
        {
            ...
            element = new EffectXmlElement
            {
                Type = EffectXmlElementType.Element,
        ...
        private bool TryReadTag(out EffectXmlElement element)
        {
            ...
            if (Peek() == '?')
            {
                ...
                element = new EffectXmlElement
                {
                    Type = EffectXmlElementType.Instruction,
                    Section = section,
            ...
            if (StartsWith("!--"))
            {
                ...
                element = new EffectXmlElement
                {
                    Type = EffectXmlElementType.Comment,
                    Section = section,
            ...
            if (insertEnd)
            {
                _pending.Enqueue(new EffectXmlElement
                {
                    Type = EffectXmlElementType.End,
                    Section = CurrentSection(),
                    Value = name
            ...
            element = new EffectXmlElement
            {
                Type = EffectXmlElementType.Start,
                Section = section,
                Value = name,

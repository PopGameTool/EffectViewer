        ...
        public void Draw(EffectViewer.TodLib.Graphics.Graphics g, bool theParentHidden)
        {
            ...
            for (int i = 0; i < mNumEffects; i++)
            {
                ...
                if (!theParentHidden || !aAttachEffect.mDontDrawIfParentHidden)
                {
                    ...
                    case EffectType.Particle:
                    {
                        TodParticleSystem aParticleSystem = aParticleHolder.mParticleSystems.DataArrayTryToGet((ParticleSystemID)aAttachEffect.mEffectID);
                        if (aParticleSystem != null)
                        {
                    ...
                    case EffectType.Trail:
                    {
                        EffectViewer.TodLib.Trail.Trail aTrail = aTrailHolder.mTrails.DataArrayTryToGet((TrailID)aAttachEffect.mEffectID);
                        if (aTrail != null)
                        {
                            aTrail.Draw(g);
                        }
